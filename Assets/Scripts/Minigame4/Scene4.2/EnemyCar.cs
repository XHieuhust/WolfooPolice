...
public class EnemyCar : MonoBehaviour
{
    ...
    void RotateWheelCar()
    {
        ...
        wheelCar1.transform.eulerAngles += new Vector3(0, 0, speedRotate);
        wheelCar2.transform.eulerAngles += new Vector3(0, 0, speedRotate);
    }
    private void Update()
    {
        if (!isIllegal)
        {
            rigidEnemyCar.velocity = new Vector2(speedAllow, 0);
            
        }
        else
        {
            rigidEnemyCar.velocity = new Vector2(speedIllegal, 0);
        }
        RotateWheelCar();
        CheckDestroy();
    }
